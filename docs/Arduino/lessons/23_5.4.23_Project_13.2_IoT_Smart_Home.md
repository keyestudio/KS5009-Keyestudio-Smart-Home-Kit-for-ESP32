### 5.4.23 Proyecto 13.2 Hogar Inteligente IoT


#### **1. Descripción**

El hogar inteligente IoT se conecta al WiFi familiar, y el teléfono móvil utilizado para la operación también debe estar conectado al mismo WiFi.

Además, el hogar inteligente también puede conectarse al hotspot del teléfono móvil. Si la conexión es exitosa, la LCD1602 mostrará la dirección IP. Usando la APP del teléfono para introducir la IP correspondiente para la comunicación se puede lograr el control desde la APP de las diversas funciones del hogar inteligente.


#### **2. Código de prueba**

⚠️ \ **ATENCIÓN:**\  Después de abrir el archivo de código, necesita modificar el nombre y la contraseña de WiFi a los que la placa de desarrollo ESP32 debe conectarse. Reemplace `ChinaNet-2.4G-0DF0` y `ChinaNet@233` por su propio nombre de WiFi y contraseña respectivamente. Debe hacer esto antes de subir el código; de lo contrario, la placa ESP32 no podrá conectarse a la red.

```c
const char* ssid = "ChinaNet-2.4G-0DF0";  // Enter your own WiFi name
const char* pwd = "ChinaNet@233"; // Enter your own WiFi passwords
```
⚠️ **NOTA: Por favor asegúrese de que el nombre y la contraseña de WiFi en el código sean los mismos que los de la red a la que están conectados su ordenador, teléfono móvil/tablet, la placa de desarrollo ESP32 y el router. Deben estar dentro de la misma red local (WiFi).**
...
⚠️ **NOTA: El teléfono móvil o tablet debe estar conectado a la placa de desarrollo ESP32 mediante el mismo WiFi. De lo contrario, no podrá acceder a la página de control. Además, cuando la placa de desarrollo ESP32 usa la función WiFi, consume mucha energía. Se requiere una fuente de alimentación DC externa para satisfacer su demanda de energía para el funcionamiento. Si no se cumple la demanda de energía, la placa ESP32 seguirá reiniciándose, lo que provocará que el código no se ejecute normalmente.**

A. Introduzca la dirección IP en la APP (La dirección IP asignada se mostrará en la LCD1602 o en la ventana del monitor serie del IDE.)

B. Haga clic en "CONNECT" para conectarse a la dirección IP.

C. La conexión exitosa se indica con la palabra "Connected" que aparece. Por favor, téngalo en cuenta.

D. Después de que la dirección IP esté conectada, haga clic en el botón correspondiente en la interfaz de la APP para controlar el funcionamiento del módulo sensor correspondiente.

![imagen71](../media/a94cd80683c4eecb3c1bcabd4a60747d.png)