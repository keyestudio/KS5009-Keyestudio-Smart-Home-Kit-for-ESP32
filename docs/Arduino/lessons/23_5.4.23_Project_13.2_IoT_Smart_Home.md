### 5.4.23 Projet 13.2 : Maison intelligente IoT


#### **1. Description**

La maison intelligente IoT se connecte au WiFi familial, et le téléphone mobile utilisé pour l'opération doit également être connecté au même WiFi.

De plus, la maison intelligente peut également se connecter au hotspot du téléphone mobile. Si la connexion réussit, le LCD1602 affichera l'adresse IP. En saisissant l'adresse IP correspondante dans l'APP du téléphone pour communiquer, il est possible de réaliser le contrôle via l'APP des différentes fonctions de la maison intelligente.
...
```
⚠️ **REMARQUE : Veuillez vous assurer que le nom et le mot de passe WiFi dans le code sont les mêmes que ceux du réseau auquel sont connectés votre ordinateur, votre téléphone mobile/tablette, la carte de développement ESP32 et le routeur. Ils doivent être sur le même réseau local (WiFi).**
...
⚠️ **Remarque : Le téléphone mobile ou la tablette doit être connecté(e) à la carte de développement ESP32 via le même WiFi. Sinon, il/elle ne pourra pas accéder à la page de contrôle. De plus, lorsque la carte ESP32 utilise la fonction WiFi, elle consomme beaucoup d'énergie. Une alimentation DC externe est requise pour satisfaire ses besoins en énergie lors du fonctionnement. Si l'alimentation est insuffisante, la carte ESP32 se réinitialisera continuellement, ce qui empêchera le code de fonctionner normalement.**

A. Saisissez l'adresse IP dans l'APP (L'adresse IP attribuée sera affichée sur le LCD1602 ou dans la fenêtre du moniteur série de l'IDE.)

B. Cliquez sur "CONNECT" pour vous connecter à l'adresse IP.

C. La connexion réussie est indiquée par l'apparition du mot "Connected". Veuillez y prêter attention.

D. Après la connexion à l'adresse IP, cliquez sur le bouton correspondant de l'interface APP pour activer le module de capteur correspondant.

![image71](../media/a94cd80683c4eecb3c1bcabd4a60747d.png)