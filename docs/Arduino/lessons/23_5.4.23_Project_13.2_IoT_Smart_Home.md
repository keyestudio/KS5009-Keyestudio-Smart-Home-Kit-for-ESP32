...
Het IOT-smart home maakt via WiFi verbinding met het thuisnetwerk, en de mobiele telefoon die voor bediening wordt gebruikt, moet ook verbonden zijn met hetzelfde WiFi.

Bovendien kan het smart home ook verbinden met de hotspot van de mobiele telefoon. Als de verbinding succesvol is, zal de LCD1602 het IP-adres weergeven. Door in de telefoon APP het corresponderende IP in te voeren voor communicatie, wordt het mogelijk om via de APP verschillende functies van het smart home te bedienen.


#### **2. Testcode**

⚠️ \ **LET OP:**\  Nadat u het codebestand hebt geopend, moet u de WiFi-naam en het wachtwoord aanpassen waarmee het ESP32 development board verbinding moet maken. Vervang `ChinaNet-2.4G-0DF0` en `ChinaNet@233` door uw eigen WiFi-naam en wachtwoord. U moet dit doen voordat u de code uploadt; anders kan het ESP32-bord geen verbinding maken met het netwerk.

```c
const char* ssid = "ChinaNet-2.4G-0DF0";  // Enter your own WiFi name
const char* pwd = "ChinaNet@233"; // Enter your own WiFi passwords
```
⚠️ **OPMERKING: Zorg ervoor dat de WiFi-naam en het wachtwoord in de code hetzelfde zijn als het netwerk waarmee uw computer, mobiele telefoon/tablet, ESP32 development board en router verbonden zijn. Ze moeten zich binnen hetzelfde lokale netwerk (WiFi) bevinden.**
...
⚠️ **Opmerking: De mobiele telefoon of tablet moet via hetzelfde WiFi verbonden zijn met het ESP32 development board. Anders kan de bedieningspagina niet worden bereikt. Daarnaast verbruikt het ESP32 development board bij gebruik van de WiFi-functie veel stroom. Een externe DC-voeding is nodig om in de vereiste voedingsbehoefte te voorzien. Als aan de voedingsbehoefte niet wordt voldaan, zal het ESP32-bord steeds opnieuw resetten, waardoor de code niet normaal wordt uitgevoerd.**

A. Voer het IP-adres in in de APP (het toegekende IP-adres wordt weergegeven op de LCD1602 of in het seriële monitorvenster van de IDE).

B. Klik op "CONNECT" om verbinding te maken met het IP-adres.

C. Een succesvolle verbinding wordt aangegeven door het verschijnen van het woord "Connected". Let hierop.

D. Nadat het IP-adres is verbonden, klikt u op de betreffende knop in de APP-interface om het bijbehorende sensormodule te bedienen.

![afbeelding71](../media/a94cd80683c4eecb3c1bcabd4a60747d.png)